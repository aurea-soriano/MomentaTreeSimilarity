...
class MinMovingEuclidean:
    ...
    def calculate(self,  dense_vector1, dense_vector2):
        ...
        for i in range(0, len(vector1)):
            min_value = float("inf")
            begin_value = max(prev, i - self.window_size)
            end_value = min(len(vector2) - 1, i + self.window_size)
...
def main():
    abstract_dissimilarity = MinMovingEuclidean()
